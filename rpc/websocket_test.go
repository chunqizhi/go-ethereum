...
func TestClientWebsocketPing(t *testing.T) {
	...
	// This is important for reproducing https://github.com/chunqizhi/go-ethereum/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	...
	const (
		subResp   = `{"jsonrpc":"2.0","id":1,"result":"Gs00"}`
		subNotify = `{"jsonrpc":"2.0","method":"eth_subscription","params":{"subscription":"Gs00","result":1}}`
	)
