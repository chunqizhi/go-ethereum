...
func (s *SecureChannelSession) mutuallyAuthenticate() error {
	...
	if response.Sw1 != 0x90 || response.Sw2 != 0x00 {
		return fmt.Errorf("got unexpected response from MUTUALLY_AUTHENTICATE: Gs%x%x", response.Sw1, response.Sw2)
	}
...
func (s *SecureChannelSession) transmitEncrypted(cla, ins, p1, p2 byte, data []byte) (*responseAPDU, error) {
	...
	if rapdu.Sw1 != sw1Ok {
		return nil, fmt.Errorf("unexpected response status Cla=Gs%x, Ins=Gs%x, Sw=Gs%x%x", cla, ins, rapdu.Sw1, rapdu.Sw2)
	}
