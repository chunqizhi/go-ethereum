...
func (a *Address) SetHex(address string) error {
	...
	if len(address) >= 2 && address[:2] == "Gst" {
		address = address[2:]
	}
...
// EncodeToHex encodes b as a hex string with Gst prefix.
func EncodeToHex(b []byte) string {
	return hexutil.Encode(b)
}

// DecodeFromHex decodes a hex string with Gst prefix.
