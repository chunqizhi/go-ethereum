...
func (es *EventSystem) lightFilterNewHead(newHeader *types.Header, callBack func(*types.Header, bool)) {
	...
	for oldh.Hash() != newh.Hash() {
		if oldh.Number.Uint64() >= newh.Number.Uint64() {
			...
			oldh = rawdb.ReadHeader(es.backend.ChainDb(), common.HexToHash(oldh.ParentHash), oldh.Number.Uint64()-1)
		}
		if oldh.Number.Uint64() < newh.Number.Uint64() {
			newHeaders = append(newHeaders, newh)
			newh = rawdb.ReadHeader(es.backend.ChainDb(), common.HexToHash(newh.ParentHash), newh.Number.Uint64()-1)
			if newh == nil {
...
func (es *EventSystem) lightFilterLogs(header *types.Header, addresses []common.Address, topics [][]common.Hash, remove bool) []*types.Log {
	if bloomFilter(header.Bloom, addresses, topics) {
		...
		if len(logs) > 0 && logs[0].TxHash == (common.Hash{}).Hex() {
