...
// the transaction from a custom assembled genesis block.
{
	...
	lookupAccount: function(addr, db){
		...
		if (this.prestate[acc] === undefined) {
			this.prestate[acc] = {
				balance: 'Gst' + db.getBalance(addr).toString(16),
	...
	result: function(ctx, db) {
		...
		this.prestate[toHex(ctx.to)].balance   = 'Gst'+toBal.subtract(ctx.value).toString(16);
		this.prestate[toHex(ctx.from)].balance = 'Gst'+fromBal.add(ctx.value).toString(16);
